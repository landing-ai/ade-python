...
fetch v1 + v2 openapi.json ─ normalize (jq -S) ─ diff vs committed specs/*.json
  ...
  └─ human review → merge → (maintainer dispatches Release, picks bump) → publish
```
...
4. API deploys to production → the release gate (every SDK-implemented path exists in the production spec) passes → a maintainer dispatches the Release workflow (choosing the bump) and merges its release PR → PyPI/npm artifact ships.

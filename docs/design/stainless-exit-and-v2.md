...
4. API deploys to production → the release gate (every SDK-implemented path exists in the production spec) passes → a maintainer dispatches the Release workflow (choosing the bump) → PyPI/npm artifact ships.

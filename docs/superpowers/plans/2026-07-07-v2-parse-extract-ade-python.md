...
- **Environment matrix** (verbatim):
  ...
  > **V2 host (live-verified 2026-07-08):** there is a spec-host vs API-host split. The **customer V2 API** (what the SDK calls, bearer auth) is `api.ade.[env].landing.ai` — the AIDE gateway routes the bare `/v1/*`,`/v2/*` paths there; that's what `V2_ENVIRONMENTS` uses. The V2 **spec** (for codegen/pipeline) is published at `aide.[env]/openapi.json` ("AIDE Gateway", 10 paths), but that host's *API* paths are staff-Google-SSO-gated, so a customer bearer call to `aide.[env]` fails (GSE 400). `api.ade.[env]`'s *published* openapi.json is VTRA's V1/tools spec — which is why the spec comes from `aide` while the callable endpoint is `api.ade`; don't conflate the two. Proven by a live staging smoke test (real bearer key): `api.ade.staging` → all V2 checks pass; `aide.staging` → all fail (SSO). (An interim draft wrongly used `aide.[env]` for the API host; corrected.)

...
- `strict` and `grounding` on `/v2/extract` and `/v2/extract/jobs` → `options.strict` and
  ...
  `body["options"] = {...}` drops whichever landed first); coerce with `bool()` / `Boolean()` rather
  than truthiness, because `grounding=False` is the only value that flag is ever passed; and an
  omitted or `None`/`undefined` value leaves its key out so the server default applies (`strict`
  false, `grounding` true), with `options` omitted entirely when neither is given.

...
def test_parse_atomic_grounding_confidence(staging_client: LandingAIADE) -> None:
    # `min_ocr_confidence` is an optional per-grounding OCR score in [0, 1]. On
    ...
    # it appears (atomic segments and node-level grounding alike). The pre-rename
    # `confidence` field is checked the same way for older gateway responses.
    pdf = Path(__file__).parent / "sample.pdf"
    ...
    def _check(grounding: Optional[V2ParseNodeGrounding]) -> None:
        if grounding is None:
            return
        if grounding.min_ocr_confidence is not None:
            assert 0.0 <= grounding.min_ocr_confidence <= 1.0
